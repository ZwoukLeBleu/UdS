...
def bode1(w, mag, phlin, title):
    ...
    # MON CODE DE SAUCE
    # ax[0].axis([1000, 25000, -50, 10])
    # ax[0].axhline(y=-3, color='black', linestyle='--')
    # idx = np.argmin(np.abs(mag + 3))
    # freq_3db = w[idx]
    # ax[0].axvline(x=freq_3db, color='black', linestyle='--')
    # ax[0].text(freq_3db, -3, f'{freq_3db:.2f} Hz', color='black', verticalalignment='bottom')
    # FIN CODE 
...
def bodeplot(b, a, title, freq):
    ...
    fig, ax = bode1(w, mag, phlin, title) # /2/np.pi
    
    # MON CODE
    # ax[0].axis([1000, 25000, -50, 10])
    ax[0].axhline(y=-3, color='black', linestyle='--')
    # idx = np.argmin(np.abs(mag + 3))
    freq_3db = freq*2*np.pi
    ax[0].axvline(x=freq_3db, color='black', linestyle='--')
    ax[0].text(freq_3db, -3, f'w0={freq_3db:.2f} Hz', color='black', verticalalignment='bottom')
    ax[0].text(freq+50, -10, f'f={freq:.2f} Hz', color='red', verticalalignment='top')
    if freq == -1:
        ax[0].axis([2, 30000*2*np.pi, -1.2, 1.2])
    # FIN CODE
